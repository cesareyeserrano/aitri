...
export default {
    ...
    if ('quality_gates' in d && d.quality_gates != null) {
      ...
      d.quality_gates.forEach((g, i) => {
        if (!g || typeof g !== 'object' || Array.isArray(g))
          throw new Error(`quality_gates[${i}] must be an object with a "command" string`);
        if (typeof g.command !== 'string' || !g.command.trim())
          throw new Error(`quality_gates[${i}] is missing a non-empty "command" (the shell command Aitri runs and gates on its exit code)`);
  ...
  buildBriefing({ dir, inputs, feedback, failingTests, artifactsBase, bestPractices, scopeVerb = '', scopeArg = '' }) {
