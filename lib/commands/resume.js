...
export function cmdResume({ dir, VERSION, args = [] }) {
  ...
  if (project.versionMismatch || project.versionMissing) {
    ...
    note(``);
    note(`  aitri normalize --init    # baselines post-build code drift`);
    note(``);
  ...
  if (root.normalizeState === 'pending') {
    ...
    note(`To close the cycle, run \`aitri normalize --resolve\` — it advances the baseline for refactor / already-registered changes.`);
    note(`Route any fr-change or new-feature through the pipeline instead.`);
    note(`Plain \`aitri normalize\` only re-displays the briefing; it does not advance the baseline.`);
    ...
    note(`Run \`aitri normalize\` to classify and route each change, then \`aitri normalize --resolve\` to close.`);
  }
