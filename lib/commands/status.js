...
function emitText(snapshot) {
  const { project, pipelines, bugs, backlog, nextActions, health, normalize } = snapshot;
...
function emitJson(snapshot) {
  const { project, pipelines, bugs, audit, health, nextActions, snapshotVersion, backlog, normalize } = snapshot;
  const root = pipelines.find(p => p.scopeType === 'root');
