...
function emitText(snapshot) {
  ...
  // approval; deployable is the composite gate (version, drift, normalize,
  ...
  if (root.normalizeState === 'pending') {
    console.log(`\n  ⚠️  Off-pipeline changes detected — close with: aitri normalize --resolve (or route fr-changes through the pipeline)`);
