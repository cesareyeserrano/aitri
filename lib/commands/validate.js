...
import { fixedBugsWithoutTC } from './bug.js';
...
export function cmdValidate({ dir, args = [] }) {
  ...
  // Bug warnings — fixed bugs with no TC reference (non-blocking)
  const untrackedFixes = fixedBugsWithoutTC(dir, config);
  for (const b of untrackedFixes) {
    console.log(`  ⚠️  ${b.id}: marked fixed but no TC reference — run: aitri bug verify ${b.id} --tc TC-NNN`);
  }
