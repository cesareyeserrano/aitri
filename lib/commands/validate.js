...
function emitText(snapshot, { explain }) {
  ...
  const found          = allDeploy.filter(f => fs.existsSync(path.join(project.dir, f)));
  const missingReq     = deployRequired.filter(f => !fs.existsSync(path.join(project.dir, f)));
  const absentOpt      = deployOptional.filter(f => !fs.existsSync(path.join(project.dir, f)));
  ...
  if (features.length) {
    ...
    if (featureVerifyCount > 0 && tests?.totals?.total > 0) {
      ...
      console.log(`  Σ all pipelines: Passed (${t.passed}/${t.total})`);
    }
  }
}
