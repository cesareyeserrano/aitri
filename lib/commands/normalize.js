...
// rc.3 — Hub canary 2026-05-13: features/<name>/spec/ and features/<name>/.aitri
// are sub-pipeline artifacts governed by the feature's own gate, not the parent
// build baseline. Excluding them symmetrically with root spec/ + .aitri removes
// false-positive entries from normalize's file list. Shared code that the feature
// contributes (lib/, tests/ outside the feature dir) stays in scope — that's a
// legitimate parent baseline change and the operator confirms it via the
// existing --resolve TTY gate.
function isFeaturePipelineArtifact(relPath) {
  return /^features\/[^/]+\/(spec\/|\.aitri(\/|$))/.test(relPath);
}
