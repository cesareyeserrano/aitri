...
function askChecklist(phase, key, px) {
  ...
  if (answer !== 'y' && answer !== 'yes') {
    process.stderr.write(
      ...
      `   Tip: run 'aitri ${px}run-phase ${key}' to see the checklist.\n`
    );
    process.exit(1);
  }
}
...
export function cmdApprove({ dir, args, err, featureRoot, scopeName }) {
  ...
  if (!completed.has(phase)) {
    err(`Phase ${key} has not been validated.\nRun: aitri ${px}complete ${key}  (must pass before approving)`);
  }
  ...
  if (driftDetected) {
    if (!process.stdin.isTTY) {
      process.stderr.write(
        ...
        `   Run 'aitri ${px}approve ${key}' manually in your terminal after reviewing the artifact.\n`
      );
      process.exit(1);
    }
  ...
  if (OPTIONAL_PHASES.includes(phase)) {
    ...
    if (phase === 'ux' && approved.has(1)) {
      ...
      console.log(`  aitri ${px}run-phase architecture\n`);
      ...
      if (config.verifyPassed) {
        console.log(`  aitri ${px}run-phase deploy\n`);
        console.log(`Verification passed and code review approved — proceed to Deploy (DevOps).`);
      } else if (approved.has(4)) {
        console.log(`  aitri ${px}verify-run\n`);
        console.log(`Build approved — run the test suite next, then aitri ${px}verify-complete to unlock Deploy.`);
      } else {
        console.log(`  aitri ${px}run-phase build\n`);
        console.log(`Complete and approve Build (Implementation) before proceeding.`);
      }
      console.log(bar);
    } else {
      console.log(`\n→ Continue with optional phases or run: aitri ${px}run-phase requirements`);
    }
  } else if (phase === 4) {
    ...
    console.log(`aitri ${px}verify-run executes your test suite — then aitri ${px}verify-complete unlocks Deploy.`);
    console.log(bar);
  } else if (phase < 5) {
    ...
    if (phase === 1) {
      ...
      try {
        ...
        process.stderr.write(
          ...
          `  If your project has UX or visual requirements, run: aitri ${px}run-phase ux\n`
        );
      }

      if (uxRequired) {
        console.log(`  aitri ${px}run-phase ux\n`);
        ...
        console.log(`After aitri ${px}approve ux → the pipeline will continue to Architecture.`);
        console.log(bar);
        return;
      }
    }

    console.log(`  aitri ${px}run-phase ${nextKey}\n`);
    console.log(`Do NOT choose a route, skip phases, or implement anything yet.`);
