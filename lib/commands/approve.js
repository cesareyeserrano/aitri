...
import { loadConfig, saveConfig, hashArtifact } from '../state.js';
...
export function cmdApprove({ dir, args, err }) {
  ...
  try {
    const artifactContent = fs.readFileSync(path.join(dir, p.artifact), 'utf8');
  ...
  if (OPTIONAL_PHASES.includes(phase)) {
    ...
    if (phase === 1) {
      ...
      try {
        const reqs = JSON.parse(fs.readFileSync(path.join(dir, '01_REQUIREMENTS.json'), 'utf8'));
        const approved = new Set(config.approvedPhases || []);
