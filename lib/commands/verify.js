  ...
  if (result.error?.code === 'ENOENT') {
    const hint = /pytest/.test(cmdParts[0])
      ? `  If using a Python virtualenv, the binary may not be in PATH.\n` +
        `  Override with: aitri ${px}verify-run --cmd ".venv/bin/pytest tests/ -v"\n` +
...
export function cmdVerifyComplete({ dir, err, featureRoot, scopeName }) {
  ...
  if (!fs.existsSync(resultsPath)) {
    err(`04_TEST_RESULTS.json not found.\nRun: aitri ${px}verify-run  then save the results file.`);
  }
  ...
  console.log(`  aitri ${px}run-phase 5\n`);
