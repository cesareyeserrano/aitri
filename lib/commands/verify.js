...
export function extractTCId(line) {
  // Namespace segments require at least one letter (`[A-Za-z][A-Za-z0-9]*`)
  // so the regex distinguishes namespace from the final numeric block. Pure
  // numeric segments would be ambiguous with `(\d+)` and collapse the parse.
  const m = line.match(
