...
export function buildPipelineEntry(pipelineDir, scope) {
  ...
  // Latest verify-run summary, derived from events (alpha.12).
  // verifySummary is only persisted on verify-complete success, but resume must
  // also know what verify-run produced when verify-complete has not passed —
  // otherwise it loops recommending verify-run after a no-op all-skip run.
  let lastRunSummary = null;
  for (let i = (config.events || []).length - 1; i >= 0; i--) {
    const e = config.events[i];
    if (e.event === 'verify-run') {
      lastRunSummary = {
        passed:  e.passed  || 0,
        failed:  e.failed  || 0,
        skipped: e.skipped || 0,
...
function aggregateBugs(pipelines) {
  ...
  for (const pl of pipelines) {
    ...
    for (const b of bugs) {
      ...
      // as it leaves `open`/`in_progress`. Mirrors normalize.js --resolve gate.
      const isActive = b.status === 'open' || b.status === 'in_progress';
      const isBlocking = isActive && (b.severity === 'critical' || b.severity === 'high');
      if (isOpen)     open++;
      if (isBlocking) blocking++;
      if (isActive && bySeverity[b.severity] !== undefined) {
...
function computeNextActions(snapshot) {
  ...
  //
  // Suppressed when blocking bugs exist: `aitri normalize --resolve` refuses to
  // run while `bugs.blocking > 0` (see lib/commands/normalize.js gate), so
  // suggesting it here is theater — the operator follows the ladder, runs the
  // command, gets rejected, fixes bugs, returns to the ladder, sees normalize
  // suggested again (visible deadlock). The blocking-bug action already
  // surfaced above at priority 3; normalize re-emerges automatically once
  // bugs close. See BACKLOG.md "Pre-promotion findings (Codex canary 2026-05-11)"
  // P1 downstream.
  if (bugs.blocking === 0) {
    if (root && root.normalizeState === 'pending') {
      // Pending = `aitri normalize` already ran (it printed the briefing and set
      // this state). Plain `normalize` is a fixed point here — re-running it never
      ...
      actions.push({
        priority: 4,
        scope:    'root',
        command:  'aitri normalize --resolve',
      ...
      // `aitri normalize` (prints the briefing, moves state to pending). The
      // pending branch above then points to --resolve to close.
      actions.push({
        priority: 4,
        scope:    'root',
        command:  'aitri normalize',
        reason:   `${normalize.uncountedFiles} file(s) changed outside pipeline since last build approval — classify them`,
        severity: 'warn',
      });
    }
  }
  ...
  for (const pl of pipelines) {
    ...
    if (!next) continue;
    // Verify-run / verify-complete are priority 5; ordinary phase work is priority 6
