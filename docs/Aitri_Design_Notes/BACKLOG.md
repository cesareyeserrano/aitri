...
- [ ] **Phase 3b (core, delicate — fresh-head) — thin IDEA + blocking confirmation in discovery.** Thin `templates/IDEA.md` (intent + context pointers + optional success line, dropping the rich 8-section pre-structure — interacts with `phase1.js buildBriefing`'s `['Problem','Target Users','Business Rules','Success Criteria']` empty-section warning, which must be reconciled); BLOCKING confirmation of problem/success/no-go placed in `phaseDiscovery.validate` (the ADR-032 D3). Open: how the discovery-side confirmation/provenance relates to the existing phase1 `validateSeedProvenance` (inherit when discovery ran; phase1 gate as fast-path fallback for trivial projects that skip discovery); non-TTY fallback (agent conducts the confirmation) so CI never hard-stops. Files: `templates/IDEA.md`, `templates/phases/phaseDiscovery.md`, `lib/phases/phaseDiscovery.js`, `lib/phases/phase1.js`/`phase1-checks.js`. **Deferred deliberately: this is the gate-migration surgery — do it with a clear head, not at the tail of a 16-release session.**
