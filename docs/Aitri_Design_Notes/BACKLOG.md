...
### Core — Scope-blind PIPELINE INSTRUCTION + briefing instructions (Ultron canary 2026-04-27)

- [ ] **P1 — `aitri (feature) approve` emits root-style next-action commands inside feature pipelines.** Destructive risk confirmed on Ultron canary v2.0.0-alpha.4.

  Evidence (verified, not paraphrased):
  - **Reproduction.** Ultron canary running a new feature `network-monitoring` with UX/visual FRs in Phase 1. After `aitri feature approve <name> requirements`, the post-action banner emitted: `PIPELINE INSTRUCTION — your only next action is: aitri run-phase ux`. The agent followed literally and ran `aitri run-phase ux` from `/Ultron` (root), producing a briefing with `## Output: /Users/cesareyeserrano/Documents/PROJECTS/Ultron/spec/01_UX_SPEC.md` — the parent project's already-approved UX spec (6 KB, 27 Apr 09:02). Saving would have overwritten an approved root artifact with feature-scope content, leaving the parent in undetectable drift (file present, content wrong).
  - **Root cause is a missing parameter.** `cmdApprove` at `lib/commands/approve.js:169` destructures only `{ dir, args, err }`. `feature.js:67-74` builds `featureCtx = { dir, args, flagValue, err, rootDir, featureRoot }` and passes it to `cmdApprove`, but `featureRoot` is silently dropped. Without it, the PIPELINE INSTRUCTION block (`approve.js:307-376`) cannot tell root from feature scope and always prints root commands.
  - **Same defect class in adjacent surfaces:**
    - `lib/commands/approve.js:308, 317, 320, 323, 328, 334, 362, 372, 163` — every PIPELINE INSTRUCTION line.
    - `lib/commands/complete.js:80` — `Approved → aitri approve ${key}`.
    - `lib/commands/reject.js:25` — `Rerun: aitri run-phase ${key} --feedback "..."`.
    - `lib/commands/verify.js:862` — `aitri run-phase 5` after verify-complete.
    - All 11 templates in `templates/phases/*.md` carry hardcoded `aitri complete X`, `aitri approve X`, `aitri verify-run`, `Next: aitri complete X → aitri approve X` lines that ignore scope. Even an agent that ran `aitri feature run-phase <name> ux` correctly (path-substituted via `{{ARTIFACTS_BASE}}`) would be told by the briefing itself to run scope-less `aitri complete ux` and re-create the same destructive write.
  - **Why Phase 1 hid the issue.** Phase 1 in a feature works because `aitri run-phase 1` is blocked at root by the "all core phases approved" gate (`run-phase.js:115`), forcing the user into `aitri feature run-phase`. UX is in `OPTIONAL_PHASES` and that gate does not apply to it.

  Problem / Why:
  - The tool's own next-action instruction directs agents to overwrite an approved artifact in the parent project. This is not a documentation gap — it is Aitri actively misleading the agent. Agents that follow PIPELINE INSTRUCTION literally (per `~/CLAUDE.md` rule "Your only next action is the one Aitri specifies") have no defense.
  - Tier-1 evidence is direct: the bug degrades produced software because the parent's approved UX artifact gets clobbered with feature-scope content, and the drift signal in `aitri status` cannot distinguish "content was overwritten by mistake" from "content was rewritten on purpose". Recovery requires git restore or full re-run of the parent's UX phase.

  Files:
  - `lib/scope.js` (new) — exported `commandPrefix(featureRoot, scopeName)` returning `''` (root) or `feature ${scopeName} ` (feature). Single source of truth for scope-aware command construction.
  - `lib/commands/approve.js` — destructure `featureRoot` and `scopeName` (or derive scopeName from `path.basename(dir)` when `featureRoot` is set), prefix every command emitted in PIPELINE INSTRUCTION, validation hints, and trailing notes (lines 163, 307-376).
  - `lib/commands/complete.js` — same treatment for line 80 (`Approved → aitri approve ...`) and any error-path hints that emit commands (lines 26, 39, 54).
  - `lib/commands/reject.js` — line 25 (`Rerun: aitri run-phase ...`).
  - `lib/commands/verify.js` — line 862 (`aitri run-phase 5`) and any other emitted commands.
  - `lib/commands/feature.js:67-74` — pass scopeName explicitly in featureCtx (cleaner than path.basename derivation in each consumer). Field name: `scopeName: path.basename(featureDir)`.
  - `lib/phases/phase1.js` … `phase5.js`, `phaseUX.js`, `phaseDiscovery.js`, `phaseReview.js` — `buildBriefing()` accepts `scopePrefix` and forwards it to `render()` as `SCOPE_PREFIX`.
  - `lib/commands/run-phase.js:172` — pass `scopePrefix` to `buildBriefing` (computed once from `featureRoot`).
  - `templates/phases/*.md` (11 files) — every literal `aitri complete X` / `aitri approve X` / `aitri verify-run` / `aitri run-phase X` in instruction sections rewrites to `aitri {{SCOPE_PREFIX}}complete X` etc. Reference mentions ("the gate of `aitri complete N` validates...") stay scope-less because they describe the gate generically.
  - `test/commands/approve.test.js` — new assertions: `cmdApprove` invoked with `featureRoot` set produces `aitri feature run-phase <name> X` in stdout for every PIPELINE INSTRUCTION branch. Existing root-context assertions stay green (no regressions).
  - `test/commands/complete.test.js`, `test/commands/reject.test.js`, `test/commands/verify.test.js` — same parity tests for each command.
  - `test/phases/*.test.js` — assert briefing output rendered with `scopePrefix='feature foo '` contains `aitri feature complete X` not `aitri complete X` in instruction blocks.

  Behavior:
  - When `featureRoot` is undefined → all emitted commands are root-style (current behavior, no regression).
  - When `featureRoot` is set → every command-string includes the `feature <scopeName> ` prefix.
  - `{{SCOPE_PREFIX}}` is always defined in render context: `''` for root, `feature <name> ` (with trailing space) for feature. Never undefined to avoid `{{SCOPE_PREFIX}}` leaking into output as literal text.

  Decisions:
  - **Single helper, not per-command duplication.** `lib/scope.js::commandPrefix()` is the only place that constructs the prefix. All command emissions go through it. Prevents drift if scope semantics change later.
  - **Feature.js owns scopeName derivation.** Currently `featureRoot` carries the parent project dir and `path.basename(dir)` gives the feature name. Promoting `scopeName` to a first-class field in featureCtx avoids `path.basename(dir)` calls scattered across consumers.
  - **`{{SCOPE_PREFIX}}` includes its trailing space.** `aitri {{SCOPE_PREFIX}}complete X` renders as `aitri complete X` (root) or `aitri feature foo complete X` (feature) without conditional space logic at every call site.
  - **Templates change is mechanical.** The change is grep-able: every `aitri complete`, `aitri approve`, `aitri verify`, `aitri run-phase` in the instruction sections of the 11 templates becomes `aitri {{SCOPE_PREFIX}}<verb>`. Reference text describing gates stays scope-less.
  - **Do NOT add a runtime guard "block writes to parent artifacts from feature context".** Tempting, but the right fix is correctness at the instruction level — guard would paper over the bug and create a second source of truth.

  Risks & mitigations:
  - **Test surface ~30-50 new asserts.** Mitigation: parameterize one helper that runs every PIPELINE INSTRUCTION branch with `featureRoot` set/unset and asserts presence of the prefix. Avoid 30 hand-written copy-pasted tests.
  - **Existing tests asserting exact root-command strings.** Mitigation: only feature-context assertions are new; root-context assertions stay verbatim. Run `npm run test:all` after each command file is migrated to catch drift early.
  - **Templates contain instruction lines mixed with reference lines.** Mitigation: review each template by hand once; do not regex-replace blindly. The instruction sections are clearly delimited (`## Instructions`, `## Delivery Summary`).

  Acceptance:
  - On Ultron at HEAD with the new feature: `aitri feature approve network-monitoring requirements` emits `PIPELINE INSTRUCTION ... aitri feature run-phase network-monitoring ux` (not `aitri run-phase ux`).
  - `aitri feature run-phase network-monitoring ux` produces a briefing whose `## Instructions` section says `Run: aitri feature complete network-monitoring ux` and whose `## Delivery Summary` footer says `Next: aitri feature complete network-monitoring ux → aitri feature approve network-monitoring ux`.
  - Root-context invocations (`aitri approve requirements` on a non-feature project) emit unchanged root-style commands.
  - `npm run test:all` passes with new tests covering feature-context emission across approve/complete/reject/verify and all 11 phase templates.
  - `docs/integrations/CHANGELOG.md` carries `— additive` entry (display-side change, no schema impact, no breaking reader contract).

  Evidence / source: Ultron canary 2026-04-27 on v2.0.0-alpha.4, mid-session for feature `network-monitoring`. User paused on the destructive-risk path before the literal save would have overwritten `/Users/cesareyeserrano/Documents/PROJECTS/Ultron/spec/01_UX_SPEC.md`. Severity HIGH — Tier-1 (degrades produced software via undetectable artifact corruption) + generalizes to every feature pipeline with optional phases or any scope-less command emission, which is all of them today. Workaround for Ultron in-flight: `aitri feature run-phase network-monitoring ux` (path substitution is correct; only the prior PIPELINE INSTRUCTION was wrong).
